...
copr_project_name=unison
rpm_name=unison
arch=x86_64

copr_project_description="Unison is a file-synchronization tool for OSX, Unix, and Windows."
...
download_source_files() {
  ...
  mkdir -p ${topdir}/SOURCES/${rpm_name}
  for source_url in $source_urls; do
    source_file=${source_url##*/}
    (cd ${topdir}/SOURCES/${rpm_name} && if [ ! -f ${source_file} ]; then curl -sLO ${source_url}; fi)
  done
}
